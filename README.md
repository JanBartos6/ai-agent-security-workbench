...
- Validation-fill + replay-safe sizing + latency-split routing included at `attacks/05_validation_fill/attack.py` (port of the proven ~84-89 public architecture, now extended with hosted-proven duplicate K8 paths). Current public anchor is GPT-OSS duplicate K8 with hosted online selector plus Gemma R57 duplicate K8; `scripts/check_hosted_ablation_status.py` tracks this as submission `55727872`, public `112.970`. The known-good fallback remains GPT numeric duplicate K8 plus Gemma R57, while speculative overfill, chain2/static replacements, and private CONFUSED_DEPUTY hedge candidates are config-gated. Live-fill loop verification uses `scripts/verify_fill.py`; per-candidate token/latency measurement uses `scripts/measure_tokens.py`.
...
.venv/Scripts/python.exe scripts/audit_hedge_mix.py --hedges 1
